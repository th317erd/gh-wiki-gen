(function(factory) {
  ...
  D.setROProperty(p, 'getContextFromPath', function(path) {
    function find(context, type, names, _nameIndex, _alreadyVisisted) {
      var alreadyVisisted = _alreadyVisisted || [],
          nameIndex = _nameIndex || 0,
      ...
      if (context.parent instanceof Context) {
        if ((nameIndex + 1) >= names.length) {
          ...
          return find(context, type, names, nameIndex + 1, alreadyVisisted);
        }
      }
      ...
      for (var i = 0, il = keys.length; i < il; i++) {
        ...
        if (val instanceof Object && alreadyVisisted.indexOf(context) >= 0) {
          alreadyVisisted.push(context);

          var ret = find(val, type, names, nameIndex, alreadyVisisted);
          if (ret)
            return ret;
        }
      }
  ...
  D.setROProperty(p, 'getPath', function(filter) {
    ...
    if (path === 'type:evisit-core-js.utils.convertHTML.helper.Function' || path === 'type:evisit-core-js.utils.convertHTML')
      console.trace();
    ...
    parse: function(fileName, source) {
      ...
      var tokenizer = new D.utils.Tokenizer({
        ...
        tokenTypes: {
          'Ignore': {
            order: 9, //Not any higher than 19 or it will parse strings inside of comments
            pattern: function(input, offset) {
              ...
              if (match && match.index === offset)
                return this.skip(match[0].length);
          ...
          'RegExp': {
            order: 11,
            pattern: function parseRegExp(str, offset) {
            ...
            success: function(rawValue, expression, flags) {
              return {
                value: expression,
                rawValue: rawValue,
                flags: flags
