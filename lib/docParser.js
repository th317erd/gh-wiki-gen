  ...
  function joinKeys() {
	...
	DocParserCommand.prototype = {
		appendTo: function(name, value) {
			var val = D.get(this.output, name);
			...
			D.set(this.output, name, val);

			return this;
		},
		get: function(name) {
			return D.get(this.output, name);
		},
		set: function(name, value) {
      this.output.block = true;
			D.set(this.output, name, value);
			return this;
		},
    getParent: function(name) {
      var docOutput = this.parser.output;
      var parts = [];

      if (this.context['base'])
        parts.push(this.context['base']);

      if (this.output['parent'])
        parts.push(this.output['parent']);

      parts.push(name);

      return D.get(docOutput, parts.join('.'));
    },
    setParent: function(name, value) {
      var docOutput = this.parser.output;
      var parts = [];
      ...
      D.set(docOutput, parts.join('.'), value);

      return this;
    },
		getContext: function(name) {
			return D.get(this.context, name);
		},
		setContext: function(name, value) {
			D.set(this.context, name, value);
			return this;
		}
	};
    ...
    buildFullPath: function(contextBlock, block) {
      var parts = [];
      if (contextBlock['base'])
        parts.push(contextBlock['base']);
    ...
    ignorePattern: function() {
      ...
      var fullPath = this.buildFullPath(outputContext, output);

      if (fullPath) {
      	var setPath = D.set(this.output, fullPath, output);
        D.set(this.output, setPath + '.path', setPath);
      }
		},
