(function(factory) {
  ...
  Block.prototype = {
    ...
    run: function(name) {
      ...
      console.log('Running command: ', nameList.join('->'));
  ...
  Generator.prototype = {
    ...
    generate: function(outputPath, docData) {
      function stringify(data, func, space, _alreadyVisisted, _fullPath) {
        function s(_add) {
          if (!space)
            return '';
        ...
        var alreadyVisisted = _alreadyVisisted || [],
            fullPath = _fullPath || [],
            chunks = [],
            keys = Object.keys(data),
            isArray = (data instanceof Array);

        alreadyVisisted.push(data);
        for (var i = 0, il = keys.length; i < il; i++) {
          var key = keys[i],
              value = data[key];

          if (func instanceof Function)
            value = func.call(data, key, value, fullPath);

          if (value instanceof Object && alreadyVisisted.indexOf(value) >= 0) {
            console.warn("Value is circular: ", fullPath.concat(key).join('.'));
            continue;
          }
          ...
          if (!isArray)
            chunks.push('"' + key + '": ' + stringify(value, func, space, alreadyVisisted, fullPath.concat(key)));
          else
            chunks.push(stringify(value, func, space, alreadyVisisted, fullPath.concat(key)));
        }

        if (chunks.length === 0)
          return (isArray) ? '[]' : '{}';

        return (isArray) ? ('[' + s(1) + chunks.join(',' + s(1)) + s(0) + ']') : ('{' + s(1) + chunks.join(',' + s(1)) + s(0) + '}');
      }
      ...
      try {
        fs.writeFileSync(path.join(this.path, 'docs.json'), stringify(docData, function(key, value) {
          if (key === 'parent')
            return undefined;
          return value;
        }, 2));  
