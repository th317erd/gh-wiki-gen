(function(factory) {
  ...
  function joinKeys() {
	...
	function DocParser() {
		...
		this.newCommand('param', 2, function(types, argName, desc) {
		...
		this.newCommand('example', 0, function(desc) {
			this.appendTo('examples', desc)
		});
	}
        ...
        tokenTypes: {
          ...
          'Example': {
            ...
            pattern: function(input, offset) {
            	var commandToken = this.previousIsType(['Command'], ['WhiteSpace', 'NewLine']);
            	if (!commandToken || commandToken.value !== 'example')
            		return;

            	console.log('Example Start: ', input.substring(offset, offset + 10));
            	var tokenizer = new D.utils.Tokenizer({
            		skipWS: false,
            		tokenTypes: {
	    						'TagEnd': {
            ...
            success: function(value) {
            	...
            	function minimumLineWhitespace(value) {
            		...
            		value.trim().replace(/^[\s]+/gm, function(match) {
            			if (match.length < min)
            				min = match.length;
            			return match;
            		});
          ...
          'CommandArgument': {
            ...
            pattern: function(input, offset) {
            	...
            	if (!this.previousIsType(['Command', 'CommandArgument'], ['WhiteSpace']))
            		return;
